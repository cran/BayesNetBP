...
#' to load customized \code{clustertree} object. In order to use this feature, a 
#' \code{clustertree} oject should be built, propagated and named \code{tree.init.p}, and 
...
#' network to faciliate visualization. The \code{Expand} function can trace the ancestors of 
...
#' subsetted graph retain all properties before subsetting, including their colors and notes.\cr
#' 
...
#' in the network, and colors the nodes in a similar manner as the function \code{PlotCGBN}. \cr
#' 
...
runBayesNetApp <- function(launch.browser=TRUE) {
  appDir <- system.file("shiny-examples", "myapp", package = "BayesNetBP")
  if (appDir == "") {
    stop("Could not find example directory. Try re-installing `BayesNetBP`.", call. = FALSE)
  }

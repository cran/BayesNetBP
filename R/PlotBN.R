...
#' from the evidence.  
#'
#' @param tree.1 a \code{clustertree} object
#' @param tree.2 a \code{clustertree} object
...
PlotCGBN <- function(tree.1, tree.2, fontsize=NULL, pbar=FALSE, plotting=TRUE) {
  ...
  # tree.1 <- tree.init.p; tree.2 <- tree.post; fontsize=NULL; pbar=FALSE; plotting=TRUE;
  ...
  tree.1 <- tree.1$cluster.tree
  tree.2 <- tree.2$cluster.tree
  ...
  for (i in 1:length(all.active)){
    ...
    post.1 <- Marginals(object.1, all.active[i])
    post.2 <- Marginals(object.2, all.active[i])
  ...
  if(length(cont.active)>0){
    ...
    par(oma=c( 0,0,0,0.5))
    color.bar(colorRampPalette(c("dodgerblue", "white", "red"))(1000), -signif(max.kl,1))
  } else {
    par(oma=c(0,0,0,4))
    Rgraphviz::plot(dag, nodeAttrs=nAttrs, main="")
    par(oma=c( 0,0,0,0.5))
    color.bar(colorRampPalette(c("white", "red"))(1000), min=0, max=signif(max.kl,1))
  }
  return(klds.0)
}

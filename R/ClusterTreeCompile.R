...
ClusterTreeCompile <- function(dag, node.class) {
  ...
  eo <- EliminationOrder(dag, discrete=node.class)
  graph.mor <- moralize(dag)
  graph.tri <- Triangulate(graph.mor, eo)
  cs <- ElimTreeNodes(graph.tri, eo)
  strongET <- StrongEliminationTree(cs, eo)
  semiET <- SemiEliminationTree(strongET, cs, node.class, eo)
  output <- list(tree.graph=semiET,
                 dag=dag,
                 cluster.sets=cs,
                 node.class=node.class,
                 elimination.order=eo)
  return(output)

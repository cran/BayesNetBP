...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$update, {
    ...
    v$pvars <- c()
    ##
    v$subset=tree.init.p$nodes
    v$pre_sub=c()
    v$sub_sel=c()
    v$cltemp=c()
    v$href=v$tree.init$nodes
    ...
    v$nodecolor <- rep("dodgerblue", length(v$tree.init$nodes)) 
    
    updateSelectInput(session, inputId="var", label = "Choose variables to set evidence", 
                      choices = v$tree.init$nodes,
                      selected = v$tree.init$nodes[1])
    
    updateSelectInput(session, inputId="abvar", label = "Choose the observed variable", 
                      choices = v$tree.init$nodes[!v$tree.init$node.class],
                      selected = v$tree.init$nodes[!v$tree.init$node.class][1])
  ...
  observeEvent(input$absorb, {
    ...
    v$nodecolor[which(v$tree.init$nodes %in% v$vars)] <- "green"
  })
  ...
  observeEvent(input$add, {
    ...
    if (node.class[input$var]) {
      v$values[length(v$values)+1] <- input$evidence
    } else {
  ...
  observeEvent(input$sg_add, {
    ...
    v$sub_sel <- c()
    v$nodecolor[which(v$tree.init$nodes %in% v$pre_sub)] <- "purple"
  })
  ...
  observeEvent(input$sg_exp, {
    ...
    for (i in 1:length(v$sub_sel)){
      ...
      temp <- union(temp, gRbase::parents(this.node, v$tree.init$dag))
    }
    
    v$sub_sel <- union(v$sub_sel, temp)
    # cat(v$sub_sel)
    v$nodecolor[which(v$tree.init$nodes %in% v$sub_sel)] <- "orange"
  })
  ...
  observeEvent(input$addall, {
    var0 <- input$abvar
    v$pvars <- setdiff(v$tree.init$nodes, var0)
  })
